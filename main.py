...
def save_history(user_id, original, converted, direction):
    ...
    try:
        ...
        # FAQAT ASOSIY QISMNI SAQLASH
        original_preview = original[:200] + "..." if len(original) > 200 else original
        converted_preview = converted[:200] + "..." if len(converted) > 200 else converted
        
        history[user_key].append({
            "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "original": original_preview,
            "converted": converted_preview,
            "direction": direction
        })
        
        if len(history[user_key]) > 20:  # 20 taga oshirildi
            history[user_key] = history[user_key][-20:]
...
def send_welcome(message):
    ...
    buttons = [
        ...
        InlineKeyboardButton("🔗 Ulashish", callback_data="share_info"),
    ]
...
1️⃣ *Asosiy konvertatsiya:*
   ...
   • Matn yuboring (cheksiz)
   • Uzun matnlar ham ishlaydi
...
📢 *Reklama tarqatish:* /advert (faqat admin)
"""
    ...
    try:
        if os.path.exists(HISTORY_FILE):
            ...
            if user_key in history and history[user_key]:
                history_text = "📜 *Oxirgi 20 ta konvertatsiya:*\n\n"
                
                for i, item in enumerate(reversed(history[user_key]), 1):
                    history_text += f"*{i}. {item['direction']}*\n"
...
"""
        ...
        for i, (user_id, count) in enumerate(top_users, 1):
            try:
                ...
                username = f"ID: {user_id[:6]}..."
            
            stats_text += f"{i}. {username} - {count} marta\n"
        
        bot.reply_to(message, stats_text, parse_mode='Markdown')
    except:
        bot.reply_to(message, "📊 Statistika hali to'planmagan")

# 7. FAYL QABUL QILISH (CHEKSIZ)
...
def handle_document(message):
    try:
        ...
        # Faylni o'qish (cheksiz)
        with open(temp_file, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
        
        # CHEKSIZ - faqat hajmini tekshirish
        file_size_mb = len(content) / 1024 / 1024
        if file_size_mb > 10:  # 10 MB limit
            bot.reply_to(message, "❌ Fayl juda katta (10 MB dan oshmasin)")
            os.remove(temp_file)
            return
        ...
        save_history(message.from_user.id, content[:200], converted[:200], direction)
        
    except Exception as e:
        bot.reply_to(message, f"❌ Xato: {str(e)[:200]}")

# 8. TARJIMA XIZMATI - GOOGLE TRANSLATE
try:
    from deep_translator import GoogleTranslator
    
    # Google Translate to'g'ri til kodlari
    LANGUAGES = {
        '🇺🇿 Uzbek': 'uz',
        '🇷🇺 Russian': 'ru', 
        '🇺🇸 English': 'en',
...
✨ *Qo'llab-quvvatlanadigan tillar:* 18+ til

⚡ *Xususiyatlar:*
• Cheksiz matn uzunligi
• Avtomatik til aniqlash
• Tezkor tarjima

*Diqqat:* Katta matnlar biroz vaqt olishi mumkin
"""
    ...
    def translate_callback(call):
        ...
        msg = bot.send_message(
            ...
            f"🌍 *Tanlangan til:* {lang_name}\n\nTarjima qilish uchun matn yuboring (cheksiz):",
            parse_mode='Markdown'
        )
    ...
    def process_translation(message, lang_code, lang_name):
        try:
            ...
            if len(text) > 4000:
                ...
                bot.send_message(message.chat.id, f"📏 Matn {len(chunks)} qismga bo'lindi. Tarjima qilinmoqda...")
                    ...
                    try:
                        ...
                        if len(chunks) > 1:
                            bot.send_message(message.chat.id, 
                                f"✅ *{i}-qism tarjima qilindi*\n\n{translated[:500]}...",
                                parse_mode='Markdown'
                            )
                    except Exception as e:
                        all_translated.append(f"[Xato: {str(e)[:100]}]")
                
                translated_text = "".join(all_translated)
            else:
                # Oddiy tarjima
                translator = GoogleTranslator(source='auto', target=lang_code)
            ...
            try:
                source_lang = GoogleTranslator(source='auto', target='en').detect(text[:100])
            except:
                source_lang = "auto"
            ...
            if len(translated_text) > 4000:
                # Bo'laklarga bo'lish
                translated_chunks = [translated_text[i:i+4000] for i in range(0, len(translated_text), 4000)]
                
                for i, chunk in enumerate(translated_chunks, 1):
                    if i == 1:
                        caption = f"🌐 *Tarjima natijasi ({lang_name})*\n\n{chunk}"
                    else:
                        caption = f"📄 *Davomi ({i}/{len(translated_chunks)})*\n\n{chunk}"
...
"""
            ...
            if "429" in error_msg:
                ...
                # Boshqa tarjima usulini sinab ko'rish
                try:
                    # Ingliz tiliga tarjima qilish
                    translator = GoogleTranslator(source='auto', target='en')
                    translated = translator.translate(text[:1000])
                    bot.reply_to(message, 
                        f"🌐 *Ingliz tiliga tarjima:*\n\n{translated}\n\n"
                        f"⚠️ {lang_name} tili hozircha qo'llab-quvvatlanmaydi. Ingliz tiliga tarjima qilindi.",
                        parse_mode='Markdown'
                    )
                except:
                    bot.reply_to(message, f"❌ Tarjimada xato: Til qo'llab-quvvatlanmaydi")
            else:
                bot.reply_to(message, f"❌ Tarjimada xato: {error_msg[:200]}")

except ImportError:
    @bot.message_handler(commands=['translate'])
    def translate_command(message):
        bot.reply_to(message, 
            "❌ Tarjima xizmati hozircha mavjud emas.\n\n"
            "*Yuklash uchun:*\n"
            "1. requirements.txt ga 'deep-translator' qo'shing\n"
            "2. GitHub'ga push qiling\n"
            "3. Render avtomatik yangilaydi",
            parse_mode='Markdown'
        )
...
@bot.message_handler(commands=['broadcast', 'advert'])
        ...
        if os.path.exists(STATS_FILE):
            ...
            for i, user_id in enumerate(users, 1):
                try:
                    ...
                    # Har 50 ta xabardan keyin progress
                    if i % 50 == 0:
                        bot.send_message(
                            message.chat.id,
                            f"⏳ *Progress:* {i}/{len(users)} ({i/len(users)*100:.1f}%)"
                        )
...
def rate_share(message):
    ...
    buttons = [
        ...
        InlineKeyboardButton("🔗 Do'stga ulash", url=f"https://t.me/share/url?url=https://t.me/translater_krill_latin_krill_bot&text=Kirill-Lotin konvertatsiya boti. Cheksiz matn, tarjima xizmati va Web App bilan!"),
        InlineKeyboardButton("👥 Guruhga qo'sh", callback_data="add_to_group"),
        InlineKeyboardButton("📢 Reklama so'rov", callback_data="advert_request")
    ]
...
# 12. ASOSIY KONVERTATSIYA - CHEKSIZ
@bot.message_handler(func=lambda message: True)
def convert_text(message):
    """Asosiy konvertatsiya funksiyasi - CHEKSIZ"""
    text = message.text
    ...
    try:
        ...
        if len(converted) > 4000:
            ...
            for i, chunk in enumerate(chunks, 1):
                if i == 1:
                    caption = f"*{direction} (qism {i}/{len(chunks)}):*\n\n`{chunk}`"
                else:
                    caption = f"*Davomi ({i}/{len(chunks)}):*\n\n`{chunk}`"
...
"""
        ...
        save_history(message.from_user.id, text[:200], converted[:200], direction)
...
*Qo'shimcha:*
• /history - Tarixni ko'rish (20 ta)
• /stats - Statistika
• /translate - Tarjima xizmati (18+ til)
• /rate - Botni baholash
• /share - Botni ulashish
"""
        bot.answer_callback_query(call.id)
    ...
    # Fayl
        ...
        bot.send_message(call.message.chat.id, 
            ...
            "📎 *Fayl yuboring va bot avtomatik ishlaydi*",
            parse_mode='Markdown')
    ...
    # Reklama (barcha uchun)
        ...
        bot.send_message(call.message.chat.id,
            ...
            "4. Web App dan foydalaning: /webapp\n\n"
            "✨ *Xususiyatlar:*\n"
            "• Cheksiz matn konvertatsiyasi\n"
            "• 18+ tilda tarjima\n"
            "• Fayllarni qabul qilish\n\n"
    ...
    # Baholash
        ...
        bot.answer_callback_query(call.id)
        rate_share(call.message)
    
    # Ulashish
    elif call.data == "share_info":
        bot.answer_callback_query(call.id)
        rate_share(call.message)
    ...
    # Reklama so'rovi
        ...
        if is_admin(call.from_user.id):
            ...
            bot.answer_callback_query(call.id, "❌ Reklama faqat adminlar uchun!", show_alert=True)
